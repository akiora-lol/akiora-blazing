...
use proto_build::game::game::{
    self,
    game_service::GameService as GrpcGameService,
    CreateGameRequest, GameResponse, GetGameRequest, GetGamesRequest,
    ManyGamesResponse, UpdateGameResultRequest,
};
...
impl GrpcGameService for GrpcGameServiceImpl {
    async fn create_game(
        ...
        let draft = req.draft.map(|d| Draft {
            history: d.history.into_iter().map(|cmd| Command(
                match team_from_proto(cmd.team) {
                    Team::Red(id) => Team::Red(id),
                    Team::Blue(id) => Team::Blue(id),
                },
                match cmd.action {
                    Some(proto_build::game::game::Action { action_type: Some(action) }) => {
                        match action {
                            proto_build::game::game::action::ActionType::Pick(p) => Action::Pick(p as usize),
                            proto_build::game::game::action::ActionType::Ban(b) => Action::Ban(b as usize),
                        }
                    }
                    _ => Action::Pick(0),
                }
            )).collect(),
            picks: d.picks.into_iter().map(|lock| ChampLock {
                champion_id: lock.champion_id as u8,
                player: Uuid::parse_str(&lock.player_id).unwrap_or_default(),
            }).collect(),
            forbidden_champions: bitvec::array::BitArray::new(d.forbidden_champions.try_into().unwrap_or([0u8; 30])),
        });
        
        let game = self.domain_service.create(id, game_series_id, draft)
            .await
    ...
    async fn get_game(
        ...
        let game = self.domain_service.get_by_id(id)
            .await
            .map_err(|e| Status::internal(e.to_string()))?
    ...
    async fn get_games(
        ...
        let games = if !ids.is_empty() {
            ...
            let gs_id = Uuid::parse_str(&game_series_id)
                .map_err(|_| Status::invalid_argument("Invalid game_series_id"))?;
            self.domain_service.get_by_game_series_id(gs_id)
                .await
                .map_err(|e| Status::internal(e.to_string()))?
        } else {
            vec![]
    ...
    async fn update_game_result(
        ...
        let results: Vec<TeamParticipant> = req.results.into_iter().map(|r| {
            TeamParticipant {
                ...
                users: r.user_ids.iter()
                    .filter_map(|id| Uuid::parse_str(id).ok())
                    .collect(),
            }
        }).collect();
        
        let game = self.domain_service.update_result(id, results)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn finish_game(
        ...
        let game = self.domain_service.finish(id)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
...
fn game_to_proto(game: crate::domain::models::Game) -> GameResponse {
    ...
    GameResponse {
        ...
        draft: game.draft().map(|d| game::Draft {
            history: d.history.iter().map(|cmd| game::Command {
                team: match &cmd.0 {
            ...
            picks: d.picks.iter().map(|lock| game::ChampLock {
        ...
        results: game.results().map(|results| {
            results.iter().map(|r| proto_build::common::TeamParticipant {
        ...
        start: game.start().map(|t| t.into()),
        end: game.end().map(|t| t.into()),
        status,
    }
}
