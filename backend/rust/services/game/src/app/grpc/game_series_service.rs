...
use proto_build::game::gameseries::{
    CreateGameSeriesRequest, GameSeriesResponse, GetGameSeriesRequest, ManyGameSeriesResponse,
    game_series_service_server::GameSeriesService as GrpcGameSeriesService,
};
...
impl GrpcGameSeriesService for GrpcGameSeriesServiceImpl {
    async fn create_game_series(
        ...
        let game_settings = game_settings_from_proto(
            settings
                .settings
    ...
    async fn get_game_series(
        ...
        if ids.len() == 1 {
            ...
            let game_series_list = self
                .domain_service
                .get_by_ids(ids)
                .await
                .map_err(|e| Status::internal(e.to_string()))?;

            let first = game_series_list
                .into_iter()
                .next()
                .ok_or_else(|| Status::not_found("GameSeries not found"))?;
    ...
    async fn get_game_series_list(
        ...
        Ok(Response::new(ManyGameSeriesResponse {
            series: proto_series,
        }))
    }
...
fn game_settings_from_proto(
    settings: proto_build::game::gameseries::game_series_settings::Settings,
) -> Result<GameSettings, Status> {
    match settings {
...
fn game_series_to_proto(game_series: crate::domain::models::GameSeries) -> GameSeriesResponse {
    ...
    GameSeriesResponse {
        ...
        game_ids,
        start: game_series.start_timestamp().map(|t| t.into()),
        end: game_series.end_timestamp().map(|t| t.into()),
        status,
    }
}

fn game_series_settings_to_proto(
    settings: &GameSettings,
) -> proto_build::game::gameseries::GameSeriesSettings {
    proto_build::game::gameseries::GameSeriesSettings {
        game_type: proto_build::game::gameseries::GameType::Lol as i32,
        settings: Some(
            proto_build::game::gameseries::game_series_settings::Settings::Lol(
